...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full bg-black font-sans px-4 lg:px-10 h-auto lg:h-auto caret-transparent" ref={containerRef}>
      <div className="max-w-7xl mx-auto py-12 lg:py-20 px-2 lg:px-8">
        <h2 className="text-2xl mb-4 text-blue-100 max-w-4xl lg:text-4xl">My Certificates & Achievements</h2>
        <p className="text-blue-300 text-sm max-w-sm lg:text-base">
          A timeline of my professional certifications and key learning milestones throughout my journey.
        </p>
      </div>

      <div ref={ref} className="relative max-w-7xl mx-auto pb-12 lg:pb-20">
        {data.map((item, index) => (
          <div key={index} className="flex justify-start pt-8 lg:pt-40 lg:gap-10">
            <div className="sticky flex flex-col items-center top-20 lg:top-40 self-start max-w-xs lg:max-w-sm lg:flex-row z-40 lg:w-full">
              ...
              <h3 className="hidden text-xl font-bold text-blue-200 lg:block lg:pl-20 lg:text-5xl">{item.title}</h3>
            </div>

            <div className="relative pl-16 pr-2 w-full lg:pl-4 lg:pr-4">
              <h3 className="block text-xl mb-3 text-left font-bold text-blue-200 lg:hidden">{item.title}</h3>

              <div className="mb-4 lg:mb-6">
                <h4 className="text-lg font-semibold text-blue-300 mb-2 lg:text-xl">{item.certificateTitle}</h4>
                <p className="text-blue-100 text-sm mb-4 lg:text-base lg:mb-6">{item.description}</p>
              </div>
